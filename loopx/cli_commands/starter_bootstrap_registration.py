...
def register_starter_bootstrap_commands(subparsers: argparse._SubParsersAction) -> None:
    ...
    agent_onboard_parser.add_argument(
        "--agent-type",
        help="Agent runtime type: codex-app, codex-ide-plugin, codex-cli, claude-code, pi, manual, or other-agent.",
    )
    agent_onboard_parser.add_argument(
        "--list-agent-types",
        action="store_true",
    ...
    bootstrap_command_pack_parser.add_argument(
        "--host-surface",
        default="chat-box",
        choices=["chat-box", "codex-app", "codex-ide-plugin", "codex-ide", "codex-cli-tui", "claude-code", "pi", "shell", "http", "worker-bridge"],
